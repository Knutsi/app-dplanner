...
def render(app: QApplication, theme: Theme, out: Path) -> None:
    ...
    activity.group.showPopup()
    settle(app)
    save(activity.group.view().window(), out, "dropdown", theme, app)
    activity.group.hidePopup()
    activity.filter.set_active({"agent"})
    debounce.flush_all()
    save(page, out, "table-filtered", theme, app)
    activity.filter.menu.popup(page.mapToGlobal(activity.filter.pos()))
    settle(app)
    save(activity.filter.menu, out, "filters", theme, app)
