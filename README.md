...
├── scripts/render_graph_editor.py  the graph editor's strip, its … menu, Jump to and the Features panel — docs/screenshots/s7-graph-editor/
...
│   ├── picker.py            the fuzzy picker over rich rows — the command palette, and the graph's Jump to
...
│   │                        (canvas_toolbar.py is the strip in named bands; jump.py the rows Jump to offers;
