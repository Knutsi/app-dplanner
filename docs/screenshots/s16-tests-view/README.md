...
the window would build differently. Re-render after changing the Tests tab, the Test panel or
the category editor.
...
| `test-panel-*` | the Test panel a run is worked down from: what the test is filed under, its last result, the four result verbs, *Show Step*, and Previous/Next — with the body **rendered**, because a numbered list is a numbered list on a surface you execute from |
...
*A test is run from a panel* have the reasoning behind each.
