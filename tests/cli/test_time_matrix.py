...
def test_progress_record_writes_a_day_once_and_the_delta_reads_against_it(
    ...
    assert row["day"] == TODAY.isoformat() and len(row["stretches"]) == 2
...
def test_a_saved_snapshot_is_named_listed_compared_against_and_forgotten(staged, cli_library):
    ...
    assert listed["saved"] == [
        {"title": "Kickoff review", "note": "day one", "day": TODAY.isoformat()}
    ]
    ...
    words = f"Kickoff review ({format_date(TODAY, TODAY)})"
