...
@pytest.fixture
def library(store):
    library = Library(name="Widget", repository="git@example.com:widget.git")
...
def test_the_product_becomes_container_directories(store, library):
...
def test_ordering_lives_in_the_parent_not_in_filenames(store, library):
    meta = read(store, "projects", "discovery", "project.json")
    assert meta["children"] == ["read-the-spec", "draft-the-model"]


def test_absence_encodes_the_default(store, library):
...
def test_the_format_version_is_written_on_the_product_only(store, library):
    assert "format" in read(store, "library.json")
...
def test_what_is_written_reads_back_the_same(store, library, tmp_path):
...
def test_saving_a_reloaded_product_changes_no_bytes(store, library):
...
def test_edges_are_one_line_per_kind(store, library):
...
def test_an_edge_kind_this_build_does_not_know_survives(store, library):
...
def test_module_data_prose_and_files_sit_side_by_side(store, library):
...
def test_clearing_module_data_leaves_nothing_behind(store, library):
    step = find(library, "Read the spec")
    modules = store.storage.root / "projects/discovery/steps/read-the-spec/modules"
    library.set_module_data(step.id, "step_estimation", {"days": 3.0})
...
def test_clearing_prose_removes_the_document(store, library):
    step = find(library, "Read the spec")
...
def test_an_unknown_module_entry_is_round_tripped(store, library):
...
def test_a_file_area_this_build_does_not_know_is_left_alone(store, library):
    modules = store.storage.root / "projects/discovery/steps/read-the-spec/modules"
...
def test_removing_the_last_file_removes_the_area(store, library):
    step = find(library, "Read the spec")
...
def test_a_workspace_changed_underneath_is_noticed(store, library):
...
def test_flushing_over_someone_elses_write_is_refused(store, library):
...
def test_reopening_clears_the_refusal(store, library):
...
def test_flush_resolves_each_kind_from_one_flat_mark(store, library):
...
def test_a_removed_step_takes_its_directory_with_it(store, library):
    project = find(library, "Discovery")
...
def test_deleting_and_undoing_writes_the_subtree_again(store, library):
    project = find(library, "Discovery")
    step = find(library, "Read the spec")
...
def test_an_unlisted_directory_holding_a_step_is_adopted(store, library):
