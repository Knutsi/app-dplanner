...
class SpecsActivity(ActivityBase):
    ...
    def __init__(
        ...
        note = QLabel(
            "The documents this project answers to. Import a spec, mark its requirements "
            "with `dplanner spec mark`, and link the steps that meet them.",
            page,
        )
        ...
        add = QPushButton("Add Document…", side)
        add.clicked.connect(lambda: self._actions.run("spec.add", self._context.current()))
        ...
        self._unsubscribes = [
            ...
            product.field_changed.connect(lambda *_a: self._refresh()),
        ]
        self._refresh()
    ...
    def on_activated(self) -> None:
        # The host already cleared the selection scope; documents are not entities, so this
        # tab publishes none of its own. The entity edge is what keeps the Project verbs —
        # Add Spec Document among them — live while this tab is current.
