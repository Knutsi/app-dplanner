"""Bindable text fields: what a text widget needs to know about the product.
...
document asks for ``ModuleTextField(product, node_id, MODULE_ID)`` and gets positional
...
from dplanner.domain.model import NodeId, Product, TextEdit


class ModuleTextField:
    """One module's prose on one node, edited positionally."""

    def __init__(self, product: Product, node_id: NodeId, key: str) -> None:
        self._product = product
        self._node_id = node_id
        self._key = key

    def read(self) -> str:
        return self._product.text(self._node_id, self._key)
    ...
    def connect(self, applied: Callable[[int, str, str, object], None]) -> Callable[[], None]:
        ...
        return self._product.text_edited.connect(on_edit)
